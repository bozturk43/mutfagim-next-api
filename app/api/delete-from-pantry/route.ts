...
export async function POST(request: NextRequest) {
    ...
    if (existingItemIndex !== -1) {
        // Ürün bulunursa, items dizisinden kaldır
        items.splice(existingItemIndex, 1);

        // Güncellenmiş items'ı Firestore'a kaydet
        await updateDoc(pantryRef, { items });

        return NextResponse.json({ message: 'Ürün başarıyla pantriden kaldırıldı!' }, { status: 200 });
    } else {
        // Ürün yoksa hata döndür
        return NextResponse.json({ error: 'Kaldırılmak istenen ürün bulunamadı!' }, { status: 404 });
    }
}
